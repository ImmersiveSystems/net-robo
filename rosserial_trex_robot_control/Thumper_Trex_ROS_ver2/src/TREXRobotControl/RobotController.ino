...
unsigned long RobotController::SimpleSonarTimer;
...
void RobotController::SimpleSonarScan(trex_robot_control::TrexRobotData &robData_msg)
{
  if(millis() - SimpleSonarTimer >= 50) //i.e., perform scanning every 50 ms
  {
    SimpleSonarTimer = millis();
    unsigned int pingVal = sonar.ping();
    sonarDistance = pingVal / US_ROUNDTRIP_CM;
  }
  robData_msg.ultrasonic_dist = sonarDistance;
}
