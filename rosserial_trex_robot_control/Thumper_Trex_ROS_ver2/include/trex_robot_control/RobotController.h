...
class RobotController 
{
	private:
		...
		static unsigned long SimpleSonarTimer;
...
//		void InitIRangeData(sensor_msgs::Range &range_msg);
		...
		//--------------  using  pinMode and pulseIn inbuilt functions -----------------//
		...
		//-----------  using non-interrupt NewPing library -------------------------------//
		void SimpleSonarScan(trex_robot_control::TrexRobotData &robData_msg);

		// ------ using interrupt based NewPing feature (very handy for multiple sonars). It has conflict with PWM on pins 9 and 11 since these also use Timer2. Further invesitiagtion needed -------
