...
{// ------------------------------------------------------------ Code for Serial Communications --------------------------------------
  ...
  if (Serial.available()>1)                                   // command available
  {
    ...
    int A=Serial.read();
    int B=Serial.read();
    Serial.println("this is A and B");
    Serial.println(A);
    Serial.println(B);
    ...
    case 18498:                                            // HB - mode and PWM data for left and right motors
      data=Serial.read()  - '0';
      Serial.println("this left mode");
      Serial.println(data, DEC);
      Leftmode=data;


      data = (Serial.read()   - '0') * 100 + (Serial.read()   - '0') * 10 + (Serial.read()   - '0');      
      Serial.println("this is left pwm");
      Serial.println(data, DEC);
      LeftPWM=data;

      data=(Serial.read()   - '0');
      Serial.println("this is right mode"); 
      Serial.println(data, DEC);    
      Rightmode=data;

      data = (Serial.read()   - '0') * 100 + (Serial.read()   - '0') * 10 + (Serial.read()   - '0');      
      Serial.println("this is right pwm");
      Serial.println(data, DEC);         
      RightPWM=data;
      break;
